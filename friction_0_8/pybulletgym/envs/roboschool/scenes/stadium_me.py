...
class StadiumScene(Scene):
	...
	def episode_restart(self, bullet_client):
		...
		if self.stadiumLoaded == 0:
			...
			frictions = [0.8, 0.8]
			colors = [[0.1,0.2,0.0,0.9],[0.1,0.2,0.0,0.9]]
			
			print(self.ground_plane_mjcf)
			for i in self.ground_plane_mjcf:
				self._p.changeDynamics(i,-1,lateralFriction=frictions[i], restitution=0.5) #0.8
				self._p.changeVisualShape(i,-1,rgbaColor=colors[i])
